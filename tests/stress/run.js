...
if ( process.env.FLEET_STRESS_WRITES !== '1' || ! root?.endsWith( '/fleet-launch-load' ) || ! hubPath ) {
...
const report = { started: new Date().toISOString(), topology: '30 independent WordPress SQLite installs; same local HTTPS origin, separate databases; one Studio PHP pool for targets, separate hub pool; not a WAN benchmark.', connections: [], checkpoints: [], faults: {}, ...( resumed?.report || {} ), passed: false };
if ( report.failure ) {
...
async function main() {
	...
	async function authenticate( directory, url, id = 1 ) {
		const cookies = runSiteWp( directory, `$expires = time() + 3600; echo 'FLEET_E2E_JSON:' . wp_json_encode( array( array( 'name' => SECURE_AUTH_COOKIE, 'value' => wp_generate_auth_cookie( ${ id }, $expires, 'secure_auth' ) ), array( 'name' => LOGGED_IN_COOKIE, 'value' => wp_generate_auth_cookie( ${ id }, $expires, 'logged_in' ) ) ) );` );
	...
	async function openHub() {
		stage = `${ stage }: close windows`;
		await closeFleetWindows();
		stage = `${ stage }: open hub`;
	...
	async function checkpoint( count ) {
		...
		for ( const site of snapshot.slice( -3 ) ) {
			...
			assert.ok( rows.join( ' ' ).includes( `Client ${ Number( site.url.match( /site-(\d+)$/ )[ 1 ] ) } article` ) );
		}
	...
	try {
		...
		for ( const fixture of fixtures ) { await authenticate( fixture.directory, fixture.url ); }
		console.log( 'Authenticated 30 isolated fixture administrators.' );
		...
		for ( const fixture of fixtures ) {
			if ( sites().some( ( site ) => site.url === fixture.url && site.setup === 'ready' ) ) {
				...
				if ( fixture.index % 10 === 0 && sites().length === fixture.index && ! report.checkpoints.some( ( item ) => item.count === fixture.index ) ) { await checkpoint( fixture.index ); }
				continue;
			}
			...
			persist();
			console.log( `Connected and verified ${ fixture.index }/30 (${ ms } ms)` );
			if ( fixture.index % 10 === 0 ) { await checkpoint( fixture.index ); }
		}
		stage = 'mixed failures and scheduled checks';
		for ( const fixture of fixtures.slice( 0, 2 ) ) { runSiteWp( fixture.directory, "update_option( 'fleet_stress_unavailable', true ); echo 'FLEET_E2E_JSON:true';" ); }
		...
		for ( let pass = 0; pass < 5; pass++ ) {
			...
			persist();
			if ( state.filter( ( site ) => site.error && site.retry > Date.now() / 1000 ).length === 3 && state.filter( ( site ) => ! site.error && site.checked > 1 ).length === 27 ) { break; }
		}
		const failed = sites();
		assert.equal( failed.filter( ( site ) => site.error && site.retry > Date.now() / 1000 ).length, 3 );
		assert.equal( failed.filter( ( site ) => ! site.error && site.checked > 1 ).length, 27 );
		assert.ok( cronRuns.every( ( seconds ) => seconds < 45 ) );
		await openHub();
		await page.waitForFunction( () => document.querySelectorAll( '.fleet-native-site-card' ).length === 30 );
		assert.equal( await page.locator( '.fleet-native-site-card [data-tone="danger"]' ).count(), 3 );
		report.faults = { unavailable: 2, revoked: 1, healthy: 27, backoff: 3, cron_runs_seconds: cronRuns };
		assert.equal( browserErrors.length, 0 );
		report.passed = true;
		delete report.resumable_session;
	} catch ( error ) {
		console.log( JSON.stringify( { stage, page_path: new URL( page.url() ).pathname, notices: await page.locator( '.fleet-native-notice, .fleet-native-setup, .fleet-native-connection-review' ).allTextContents() } ) );
		throw error;
	} finally {
		await browser.close();
	}
...
run.catch( ( error ) => {
	...
	for ( const fixture of fixtures ) {
		try {
			runSiteWp( fixture.directory, `delete_option( 'fleet_stress_unavailable' ); foreach( WP_Application_Passwords::get_user_application_passwords( 1 ) as $item ) { if ( '${ appId }' === ( $item['app_id'] ?? '' ) ) { WP_Application_Passwords::delete_application_password( 1, $item['uuid'] ); } } echo 'FLEET_E2E_JSON:true';` );
	...
	fs.writeFileSync( path.join( __dirname, 'results', `load-${ Date.now() }.json` ), JSON.stringify( report, null, 2 ) + '\n' );
